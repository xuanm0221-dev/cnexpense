...
import { formatK, formatM, formatPercent, formatRangeWithCommas } from "@/lib/expense-utils";
...
export function AdSalesEfficiencyAnalysis({
  ...
  const [axisFontSize, setAxisFontSize] = useState(8);
  useEffect(() => {
    const update = () => setAxisFontSize(window.innerWidth < 640 ? 8 : window.innerWidth < 1024 ? 9 : 10);
    update();
  ...
  return (
    <div className="space-y-6">
      ...
      <Card style={{ borderColor: navyColor, borderWidth: "1px" }}>
        ...
        />
        <div className="px-5 py-3 flex flex-row justify-between items-center border-b border-gray-200">
          <h3 className="text-[16px] font-bold" style={{ color: navyColor }}>
            광고비 효율분석
          </h3>
          <button
            type="button"
        ...
        <CardContent className="pl-5">
          <div className="flex flex-wrap items-stretch gap-4">
                ...
                <>
                  ...
                  <table className="w-full text-[10px] border border-gray-200 border-collapse">
                    <thead className="bg-gray-100">
                      <tr>
                        <th className="px-1.5 py-0.5 border border-gray-200 text-left font-semibold text-gray-700">월</th>
                        <th className="px-1.5 py-0.5 border border-gray-200 text-right font-semibold text-gray-700">(FR+OR)리테일(M)</th>
                        <th className="px-1.5 py-0.5 border border-gray-200 text-right font-semibold text-gray-700">광고비(K)</th>
                      ...
                      {(() => {
                        ...
                        return (
                          <tr className="bg-slate-100 font-bold">
                            <td className="px-1.5 py-0.5 border border-gray-200 text-navy">합계</td>
                            <td className="px-1.5 py-0.5 border border-gray-200 text-right text-navy">{fmt(totalSales / 1_000_000)}</td>
                            <td className="px-1.5 py-0.5 border border-gray-200 text-right text-navy">{fmt(totalAd / 1000)}</td>
                            <td className="px-1.5 py-0.5 border border-gray-200 text-right text-navy">{roasSum.toFixed(2)}</td>
                          </tr>
                        );
                      })()}
                      {monthlyROAS.map(d => {
                        ...
                        return (
                          <tr key={d.month}>
                            ...
                            <td className="px-1.5 py-0.5 border border-gray-200 text-right text-navy font-bold">{d.roas.toFixed(2)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
            ...
            >
              <div
                className="w-12 h-12 rounded-full flex items-center justify-center text-[10px] font-bold border-2 flex-shrink-0"
                style={{
              ...
              <div className="min-w-0 space-y-1">
                ...
                <div className="text-[10px] mt-0.5" style={{ color: efficiencyGrade.color }}>
                  <span className="font-medium">판정 이유: </span>
                  {efficiencyGrade.reason}
                </div>
                <div className="text-[10px]" style={{ color: efficiencyGrade.color }}>
      ...
      <Card style={{ borderColor: navyColor, borderWidth: "1px" }}>
        ...
        <CardHeader className="pl-5">
          <CardTitle className="text-[16px] font-bold" style={{ color: navyColor }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            ...
            >
              ...
              <YAxis
                ...
                label={{ value: "ROAS", angle: -90, position: "insideLeft", style: { fontSize: 9 } }}
                stroke="#8b5cf6"
              />
              <YAxis
                yAxisId="right"
                orientation="right"
                tick={{ fontSize: axisFontSize }}
                label={{ value: "매출 (M)", angle: 90, position: "bottom", style: { fontSize: 9 }, dy: 40 }}
              ...
              <YAxis
                ...
                label={{ value: "광고비 (M)", angle: 90, position: "bottom", style: { fontSize: 9 }, dy: 40 }}
                tickFormatter={(v) => formatM(v, 0)}
                stroke="#8884d8"
                width={60}
              />
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-white p-3 border rounded shadow-lg">
                        ...
                        <p className="text-[10px]">ROAS: {data.roas.toFixed(2)}</p>
                        <p className="text-[10px]">광고비: {formatK(data.adSpend)}</p>
                        <p className="text-[10px]">매출: {formatK(data.sales)}</p>
                      </div>
                    );
                  }
                  return null;
              ...
              />
              <Legend wrapperStyle={{ fontSize: axisFontSize }} iconSize={axisFontSize} />
              <ReferenceLine
                yAxisId="left"
                y={avgROAS}
                stroke="#999"
        ...
        <Card style={{ borderColor: navyColor, borderWidth: "1px" }}>
          ...
          <CardContent className="pl-5">
            <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-3 items-stretch">
                ...
                >
                  ...
                  <XAxis
                    ...
                    tick={({ x, y, payload }) => {
                      ...
                      return (
                        <text x={x} y={y + 8} textAnchor="middle" fontSize={8} fill="#374151">
                          <tspan x={x} dy={0}>{formatRangeWithCommas(value)}</tspan>
                          <tspan x={x} dy={10}>
                            {item ? `ROAS ${item.avgROAS.toFixed(1)}` : ""}
                          </tspan>
                        </text>
                      );
                    }}
                  />
                  <YAxis
                    label={{ value: "평균 ROAS", angle: -90, position: "insideLeft", style: { fontSize: 9 } }}
                    tick={{ fontSize: axisFontSize }}
                  />
                  <Bar
                    dataKey="avgROAS"
              ...
              {/* 해석 — 차트 우측 */}
              <div className="text-[10px] bg-green-50 p-3 rounded-lg border border-green-200 self-stretch">
                <p className="font-semibold text-green-800 mb-1">해석</p>
                <p className="text-green-800 leading-tight">{optimalRange.recommendation}</p>
                <p className="text-green-700 mt-2 text-[9px] leading-tight">
      ...
      {quartileSegments.length > 0 && (
        <Card style={{ borderColor: navyColor, borderWidth: "1px" }}>
          ...
          <CardHeader className="pl-5">
            <CardTitle className="text-[16px] font-bold" style={{ color: navyColor }}>광고비 구간별 분석</CardTitle>
          </CardHeader>
          <CardContent className="pl-5">
            <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-3 items-stretch">
              <div>
                  ...
                  <table className="w-full text-[10px]">
                    <thead>
                      <tr className="border-b">
                        ...
                        <th className="text-right py-1">평균 광고비</th>
                        <th className="text-right py-1">평균 매출</th>
                        <th className="text-right py-1">매출 YoY(%)</th>
                        <th className="text-right py-1">관측 수</th>
                      </tr>
                    </thead>
                    <tbody>
                      {quartileSegments.map((seg) => (
                        <tr key={seg.label} className="border-b">
                          <td className="py-1">{seg.label}</td>
                          <td className="py-1 text-right">{formatK(seg.avgAdSpend)}</td>
                          <td className="py-1 text-right">{formatK(seg.avgSales)}</td>
                          <td className="py-1 text-right">
                            {seg.salesGrowthYoY != null ? `${seg.salesGrowthYoY.toFixed(1)}%` : "-"}
                          </td>
                          <td className="py-1 text-right">{seg.count}개월</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="space-y-2 self-stretch">
                <div className="text-[10px] bg-amber-50 p-3 rounded-lg border border-amber-200">
                  ...
                  <p className="text-amber-800 leading-tight text-[9px]">
                ...
                <div className="text-[10px] text-gray-600 bg-gray-50 p-2.5 rounded leading-tight">
        ...
        <Card style={{ borderColor: navyColor, borderWidth: "1px" }}>
          ...
          <CardHeader className="pl-5">
            <CardTitle className="text-[16px] font-bold" style={{ color: navyColor }}>
              광고비-매출 증감률 비교 (YoY)
              {elasticity !== 0 && (
                <span className="text-[10px] font-normal text-gray-600 ml-4">
          ...
          <CardContent className="pl-5">
            ...
            <div className="mt-4 text-[10px] bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-orange-800">{saturation.message}</p>
              </div>
            </div>
          </CardContent>
        </Card>
